...
const PlayStoreBadge = ({ className = "" }: PlayStoreBadgeProps) => {
  ...
  return (
    <div
      ...
      aria-label={`${t("badges.playStoreSoon")} ${t("badges.playStoreName")}`}
    >
      ...
      <div className="flex flex-col items-start leading-tight">
        <span className="text-[10px] font-medium opacity-90">{t("badges.playStoreSoon")}</span>
        <span className="text-lg font-semibold -mt-0.5">{t("badges.playStoreName")}</span>
      </div>
    </div>
  );
};
